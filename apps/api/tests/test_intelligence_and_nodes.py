...
def test_semantic_score_route_exposes_legacy_fis_scorer():
    from file_intelligence_hub.api.routes_semantic import SemanticScoreRequest, score_semantic

    response = score_semantic(
        SemanticScoreRequest(
            path="inline-theology-note.md",
            text="Grace, faith, truth, knowledge, and repentance shape this research plan.",
        )
    )

    payload = response["semantic_address"]
    assert payload["source"] == "legacy-fis-semantic-scorer"
    assert payload["coord_hash_full"]
    assert payload["meta"]["path"]
    assert payload["variables"] == ["G", "M", "E", "S", "T", "K", "R", "Q", "F", "C"]
