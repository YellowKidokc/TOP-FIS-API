...
import hashlib
import json
from core.term_expander import get_expander
import os
import re
import shutil
import sqlite3
...
# Cache for expanded domain keywords so WordNet lookups happen once per process.
_DOMAIN_EXPANSIONS: dict[str, dict[str, list[tuple[str, float]]]] = {}


def _get_domain_expansions() -> dict[str, dict[str, list[tuple[str, float]]]]:
    """Expand every keyword in DOMAIN_RULES and cache the result."""
    global _DOMAIN_EXPANSIONS
    if _DOMAIN_EXPANSIONS:
        return _DOMAIN_EXPANSIONS
    expander = get_expander()
    for domain, rules in DOMAIN_RULES.items():
        expanded: dict[str, list[tuple[str, float]]] = {}
        for kw in rules.get("keywords", []):
            terms = expander.expand(kw, use_wordnet=True, use_conceptnet=False, top_n=20)
            expanded[kw.lower()] = [(t.term, t.weight) for t in terms]
        _DOMAIN_EXPANSIONS[domain] = expanded
    return _DOMAIN_EXPANSIONS
...
def classify_by_keywords(keywords: list, ext: str) -> dict:
    ...
    for domain, rules in DOMAIN_RULES.items():
        ...
        domain_expansions = _get_domain_expansions().get(domain, {})
        ...
        for rule_kw in rules['keywords']:
            ...
            # Substring in full text
                ...
                # Expanded term match via WordNet/ConceptNet
                for expanded_term, weight in domain_expansions.get(rule_kw_lower, []):
                    if expanded_term in kw_texts:
                        score += int(8 * weight)
                        matched.append(f"≈{rule_kw}→{expanded_term}")
                        break
                    elif any(expanded_term in kw for kw in kw_texts):
                        score += int(4 * weight)
                        matched.append(f"≈{rule_kw}~{expanded_term}")
                        break
                    elif expanded_term in full_text:
                        score += int(2 * weight)
                        matched.append(f"≈{rule_kw}≈{expanded_term}")
                        break
